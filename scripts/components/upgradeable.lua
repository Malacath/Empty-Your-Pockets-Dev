...
function Upgradeable:OnLoad(date)
	if data then
...
function Upgradeable:SetDiscount(discount)
	self.discount = discount
	--can also be used to increase prices
end 

function Upgradeable:MoneyDelta(amount)
	self.money = self.money + amount
	self.inst:PushEvent("moneydelta", {money = self.money})
end

function Upgradeable:AddSource(source)
	if type(source) == "string" then
		table.insert(self.upgradesources, source)
	end
end

function Upgradeable:ApplyUpgrade(upgrade, payed)
...
function Upgradeable:HasUpgrade(upgrade)
...
function Upgradeable:RemoveCost(upgrade)
...
function Upgradeable:CanBuy(upgrade)
	if not upgrade or self.freebuymode then
		return true
	end
